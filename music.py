...
class test_COG(commands.Cog):
    ...
    @commands.command(name='disconnection')
    async def disconnection(self, ctx):
    ...
    @commands.command(name="voice_channel_id")
    async def voice_channel_id(self, ctx):
        await ctx.send(ctx.author.voice.channel.id)

    @commands.command(name="connction_test")
    async def connction_test(self, ctx):
        print("t")
        await discord.voice_client.VoiceClient.connect(self=self.author.voice.channel.id,timeout=10,self_deaf=False,self_mute=False,reconnect=False)
...
class MusicPlayer(commands.Cog, name='Music'):
    ...
    async def before_play(self, msg):
        ...
        if msg.voice_client is None:
            print("le bot se connecte au salon vocal ")
            await msg.send("the music will start soon as possible")
    ...
    async def resume(self, msg):
        ...
        if msg.author.voice is not None and msg.voice_client is not None:
            ...
            if msg.voice_client.is_paused() is True:
                ...
                await msg.send("song is resumed")
